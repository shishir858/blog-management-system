...
$uploadsDir = $projectRoot . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;
if (!is_dir($uploadsDir)) {
    mkdir($uploadsDir, 0755, true);
}

/** Web URL for a stored media path (DB: assets/uploads/...). */
$bms_media_asset_url = static function (string $filePath): string {
    $p = str_replace('\\', '/', trim($filePath));
    if ($p === '') {
        return '';
    }
...
if (isset($_GET['delete'])) {
    ...
    if (bms_can_delete_any_media()) {
        $mediaStmt = $pdo->prepare('SELECT * FROM media WHERE id = ?');
        $mediaStmt->execute([$media_id]);
    } else {
    ...
    if ($media) {
        $rel = str_replace('\\', '/', ltrim((string) $media['file_path'], '/'));
        $absFile = $projectRoot . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $rel);
        if (is_file($absFile)) {
            unlink($absFile);
        }
        ...
        <?php else: ?>
            <div class="media-grid">
                <?php foreach ($mediaFiles as $file): ?>
                    ...
                    $fullUrl = $bms_media_asset_url((string) ($file['file_path'] ?? ''));
                    $canDel = bms_can_delete_any_media() || (int) ($file['user_id'] ?? 0) === $user_id;
                    ?>
                    <div class="media-item <?= !$isImage ? 'file-icon' : '' ?>">
                        <?php if ($isImage): ?>
                            <img src="<?= htmlspecialchars($fullUrl, ENT_QUOTES, 'UTF-8') ?>" alt="<?= htmlspecialchars((string) $file['file_name'], ENT_QUOTES, 'UTF-8') ?>" class="media-preview" loading="lazy">
                        <?php else: ?>
                            <span><?= strtoupper($ext) ?></span>
                        <?php endif; ?>
                        <div class="media-overlay">
                            <a href="javascript:void(0)" onclick="copyToClipboard(<?= htmlspecialchars(json_encode($fullUrl), ENT_QUOTES, 'UTF-8') ?>)">Copy URL</a>
                            <?php if ($canDel): ?>
                            <a href="?delete=<?= (int) $file['id'] ?>" onclick="return confirm('Delete this file?');">Delete</a>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
            ...
            <table class="wp-list-table fixed striped">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $canDelRow = bms_can_delete_any_media() || (int) ($file['user_id'] ?? 0) === $user_id;
                            ?>
                            <?php if ($canDelRow): ?>
                            <a href="?delete=<?= (int) $file['id'] ?>" class="text-danger" onclick="return confirm('Delete this file?');">Delete</a>
                            <?php else: ?>
