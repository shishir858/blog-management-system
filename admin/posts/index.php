...
/* Count trash items (own posts only for authors) */
if (bms_can_edit_all_posts()) {
    $trashCount = (int) $pdo->query("SELECT COUNT(*) FROM posts WHERE status='trash'")->fetchColumn();
} else {
    $tc = $pdo->prepare("SELECT COUNT(*) FROM posts WHERE status='trash' AND user_id = ?");
    $tc->execute([$uid]);
